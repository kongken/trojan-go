...
func (c *InboundConn) Close() error {
	slog.Info("user connection closed",
		"user", c.hash,
...
func (s *Server) acceptLoop() {
	for {
		...
		if err != nil { // Closing
			slog.Error("trojan failed to accept conn", "error", err)
			select {
			case <-s.ctx.Done():
				return
			default:
		...
		go func(conn tunnel.Conn) {
			...
			if err := inboundConn.Auth(); err != nil {
				...
				slog.Warn("connection with invalid trojan header",
			...
			case Connect:
				if inboundConn.metadata.DomainName == "MUX_CONN" {
					...
					slog.Debug("trojan mux relay connection")
				} else {
					s.connChan <- inboundConn
					slog.Debug("trojan connection")
				}
			...
			case Associate:
				...
				slog.Debug("trojan udp connection")
			case Mux:
				s.muxChan <- inboundConn
				slog.Debug("trojan mux connection")
			default:
				slog.Error("unknown trojan command", "command", inboundConn.metadata.Command)
			}
		}(conn)
	}
}
...
func NewServer(ctx context.Context, underlay tunnel.Server) (*Server, error) {
	...
	if cfg.MySQL.Enabled {
		slog.Debug("mysql enabled")
		auth, err = statistic.NewAuthenticator(ctx, mysql.Name)
	} else {
		slog.Debug("auth by config file")
	...
	go s.acceptLoop()
	slog.Debug("trojan server created")
	return s, nil
}
